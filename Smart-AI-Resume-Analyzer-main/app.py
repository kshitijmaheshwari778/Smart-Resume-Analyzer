...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "🔐 LOGIN / SIGNUP": self.render_auth_page,
    ...
    def add_footer(self):
        ...
        with col2:
            ...
            <p style='text-align: center;'>
                ...
                <a href="https://www.linkedin.com/in/kshitij-dakhera//" target="_blank" style='text-decoration: none; color: #FFFFFF'>
                    <b>Kshitij Dakhera </b>
                </a>
            </p>
    ...
    def export_to_excel(self):
        ...
        try:
            resumes = list(db.resume_data.find().sort("created_at", -1))
            rows = []
            for r in resumes:
                rid = str(r['_id'])
                a = db.resume_analysis.find_one({"resume_id": rid}) or {}
                rows.append({
 ...
 #st.toast("Check out these repositories: [Awesome Hacking](https://github.com/Hunterdii/Awesome-Hacking)", icon="ℹ️")
     ...
     "124852522.jpeg")
        ...
        st.markdown(f"""
            <div class="profile-section">
                ...
                <h2 class="profile-name">Kshitij (Kshitij)</h2>
                <p class="profile-title">Full Stack Developer & AI/ML Enthusiast</p>
                <div class="social-links">
                    <a href="https://github.com/kshitijmaheshwari778" class="social-link" target="_blank">
                        <i class="fab fa-github"></i>
                    </a>
                    <a href="https://www.linkedin.com/in/kshitij-dakhera/" class="social-link" target="_blank">
                        <i class="fab fa-linkedin"></i>
                    </a>
                    <a href="mailto:kshitijmaheshwari778@gmail.com" class="social-link" target="_blank">
    ...
    def render_auth_page(self):
        """Render User Login and Signup Page"""
        apply_modern_styles()
        page_header("User Authentication", "Login or create an account to unlock all features")
        
        # Center the form and make it narrower
                ...
                with st.form("login_form"):
                    ...
                    if st.form_submit_button("Log In", type="primary", use_container_width=True):
                        if email and password:
                            success, message, user = verify_user_login(email, password)
                            if success:
                    ...
                    if st.form_submit_button("Sign Up", type="primary", use_container_width=True):
                        if new_name and new_email and new_pass:
                            if new_pass == confirm_pass:
                                success, message = create_user(new_name, new_email, new_pass)
                                if success:
    ...
    def main(self):
        ...
        with st.sidebar:
            ...
            # Navigation buttons
            restricted_pages = ["RESUME ANALYZER", "RESUME BUILDER", "JOB SEARCH", "FEEDBACK"]
            
            for page_name in self.pages.keys():
                # Only show DASHBOARD to admins
                if "DASHBOARD" in page_name and not st.session_state.get('is_admin', False):
                    continue
                
                # Hide restricted features if user is not logged in
                is_restricted = any(res in page_name for res in restricted_pages)
                if is_restricted and not st.session_state.get('is_user_logged_in', False):
                    continue
                    
                # Hide Login/Signup if user is already logged in
                if "LOGIN / SIGNUP" in page_name and st.session_state.get('is_user_logged_in', False):
                    continue

                if st.button(page_name, use_container_width=True):
                    cleaned_name = page_name.lower().replace(" ", "_").replace("🏠", "").replace("🔐", "").replace("🔍", "").replace("📝", "").replace("📊", "").replace("🎯", "").replace("💬", "").replace("ℹ️", "").replace("/", "").strip()
                    st.session_state.page = cleaned_name
                    st.rerun()
        ...
        page_mapping = {name.lower().replace(" ", "_").replace("🏠", "").replace("🔐", "").replace("🔍", "").replace("📝", "").replace("📊", "").replace("🎯", "").replace("💬", "").replace("ℹ️", "").replace("/", "").strip(): name 
